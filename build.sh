...
python -m pip install "pydantic==2.10.3" "pydantic-settings==2.6.1" "email-validator==2.1.1" python-dotenv --no-cache-dir --only-binary=:all:
python -m pip install pyjwt "python-jose[cryptography]" passlib ecdsa --no-cache-dir --only-binary=:all:
